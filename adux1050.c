/**
*\mainpage
* ADUX1050 Controller Drivr
\n
* @copyright 2014 Analog Devices Inc.
\n
* Licensed under the GPL version 2 or later.
* \date      January-2014
* \version   Driver 0.0.1
*/

/**
* \file ad1050.c
* This file is the core driver part of ADUX Capacitive sensor.
* It also has routines for interrupt handling,
* suspend, resume, initialization routines etc.
* ADUX1050 Controller Driver
* Copyright 2014 Analog Devices Inc.
* Licensed under the GPL version 2 or later.
*/
...
#define ADUX1050_Driver_Dbg(format, arg...)	pr_info("[AD7146] :"\
						       format , ## arg)
#else
#define ADUX1050_Driver_Dbg(format, arg...)	do {		\
							if (0) 	\
						} while(0)	\
#endif
...
/**
  ...
  */
static struct list_head ad7146_device_list = LIST_HEAD_INIT(adux1050_device_list);
EXPORT_SYMBOL(adux1050_device_list);
#endif

typedef int32_t (*adux1050_read_t)(struct device *, uint16_t, uint16_t*, uint16_t);
typedef int32_t (*adux1050_write_t)(struct device *, uint16_t, uint16_t*, uint16_t);
...
*/
#define ADUX1050_DEV_ID_REG		(0x0)
/**
  Power and control Register
 */
#define PWR_CTRL_REG			(0x1)
/**
  Cconversion time control Register
 */
#define CONV_TIME_CTRL_REG		(0x2)
/**
  Filter and interrupt control
 */
#define FILTER_INT_CTRL_REG		(0x3)
/**
 ...
 */
#define OFFSET_FILE_PATH "/data/misc/adux1050_dac_data"
/**
Driver name of this adux1050 driver
*/
#define DRIVER_NAME "adux1050"
/**
...
*/
#define METAL_REV_ID_MASK		(0xF0)
/**
 ADUX1050 Power MODE mask
*/
#define PWR_MODE_MASK			(0x3)
/**
 ADUX1050 NUM_STAGE mask
*/
...
/**
 Set Number of stages connected
*/
#define SET_NUM_STG(pwr,stg_cnt) 	(((pwr) & ~NUM_STAGE_MASK) | (stg_cnt<<2))
/**
 Get power mode
*/
#define GET_NUM_STG(pwr) 		(((pwr) & NUM_STAGE_MASK)>>2)
/**
...
/**
 SET AUTO_WAKE UP time 
*/
#define SET_AUTO_WAKE_TIME(t_ms,pwr)	(((pwr) & 0x0FFF) | (((t_ms)/20)<<12))
/**
 Set Timed conversion time
*/
#define SET_TIMED_CONV_TIME(t_ms,pwr)	(((pwr) & 0xFF0F) | (((t_ms)/100)<<4))
...
/**
	This is used to hold the current read interrupt status
*/
	uint16_t int_status;
/**
	This is used to hold the current read power status
*/
	unsigned short pwr_status;
#if 0
/**
	This is used to 
*/
...
static DEVICE_ATTR(ad7146_reg_read, S_IRWXUGO, show_reg_read, store_reg_read);
/*--------------------------------------------------------------*/
/*--------------------------------------------------------------*/
static ssize_t store_reg_write(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static  DEVICE_ATTR(ad7146_reg_write, S_IRWXUGO, NULL, store_reg_write);
/*--------------------------------------------------------------*/
static ssize_t redo_filp_calib(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(ad7146_filp_config, S_IRWXUGO, NULL, redo_filp_calib);
/*--------------------------------------------------------------*/
#ifdef CONFIG_STG_HYS
...
static ssize_t show_hys_percent(struct device *dev,
	struct device_attribute *attr, char *buf);
static DEVICE_ATTR(ad7146_hys_percent, S_IRWXUGO, show_hys_percent,
		   store_hys_percent);
#endif
/*--------------------------------------------------------------*/
...
static ssize_t show_stage_info(struct device *dev,
	struct device_attribute *attr, char *buf);
static DEVICE_ATTR(ad7146_stage_info, S_IRWXUGO, show_stage_info,
		   store_stage_info);
/*--------------------------------------------------------------*/
static ssize_t do_calibrate(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(ad7146_calibrate, S_IRWXUGO, NULL, do_calibrate);
/*--------------------------------------------------------------*/
static ssize_t device_force_calibrate(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(ad7146_force_calib, S_IRWXUGO, NULL, device_force_calibrate);
/*--------------------------------------------------------------*/
static ssize_t show_dev_enable(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t device_enable(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(ad7146_enable, S_IRWXUGO, show_dev_enable, device_enable);
/*--------------------------------------------------------------*/
static ssize_t show_send_event(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t send_eventout(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(ad7146_send_eventout, S_IRWXUGO, show_send_event,
		   send_eventout);
/*--------------------------------------------------------------*/
...
static ssize_t show_dac_status(struct device *dev,
	struct device_attribute *attr, char *buf);
static DEVICE_ATTR(ad7146_dac_calibrate, S_IRWXUGO, show_dac_status,
		   do_dac_calibrate);
/*--------------------------------------------------------------*/
...
static ssize_t show_dac_mid_value(struct device *dev,
	struct device_attribute *attr, char *buf);
static DEVICE_ATTR(ad7146_dac_mid_val, S_IRWXUGO, show_dac_mid_value,
		   store_dac_mid_value);
/*--------------------------------------------------------------*/
...
/**
 This is used for the checking the flow of the CDC data
 The flow can be positive ie.Increase in positive has increased CDC or
 negative if the Increase in positive produces a decreased CDC.
 or if the Sensor is in saturation the flow can't be determined
 with a simple +/- LSB of the DAC offsets
*/
static inline uint16_t check_dac_flow(struct adux1050_chip *adux1050, uint16_t current_cdc, uint16_t cur_stg)
{
	uint16_t u16_ret;
	//uint16_t u16_cur_cdc = ZERO_VAL;
	//uint16_t u16_cur_stg = ZERO_VAL;
	uint16_t u16_loop_cnt = ZERO_VAL;
	uint16_t u16_cur_offset = ZERO_VAL;
	adux1050->read(ad7146->dev, OFFSET_DAC_STG0_REG, &u16_cur_offset, R_LEN);
...
static void dac_calibration_work(struct work_struct *work)
{
	struct adux1050_chip *adux1050 = container_of(work,
						  struct adux1050_chip,
						  calib_work);
	uint16_t u16_curr_cdc = ZERO_VAL;
		...
		if ((ad7146->low_status & AD7146_BIT_MASK(sw->index)) &&
			...
			if (data_read_ctl != FULL_SCALE_VALUE ||
				...
				ad7146->write(ad7146->dev, AMB_COMP_CTRL0_REG,
					      data_read_ctl);
				msleep(MIN_FORCED_CAL_SLEEP);
				AD7146_Driver_Dbg("Force Calib for LS %x\n",
						  ad7146->low_status);
			}
			return;
		}
		...
		const struct i2c_device_id *id)
{
	...
	*/
	ad7146->send_event = DISABLE_AD7146;
